...
function draw() {
  ...
  if (iterationCount < maxIterations) {
    ...
    console.log(`=== Hill Climbing Simple (${cityCoords.length} Cities) ===`);
...
function hillClimbingStep() {
  ...
  let i = floor(random(newRoute.length));
  let j = floor(random(newRoute.length));

  [newRoute[i], newRoute[j]] = [newRoute[j], newRoute[i]]; // Swap cities

  let newDistance = calcDistanceHC(newRoute);
  if (newDistance < bestDistanceHC) {
